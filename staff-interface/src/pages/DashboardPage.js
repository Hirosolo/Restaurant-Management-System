...
const DashboardPage = () => {
  ...
  useEffect(() => {
    const fetchDailySales = async () => {
      try {
        const response = await fetch(`http://localhost:3001/api/sales/daily/${selectedYear}/${selectedMonth}`);
        if (!response.ok) {
  ...
  useEffect(() => {
    const fetchDailyRevenue = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:3001/api/orders/revenue`, {
            headers: {
  ...
  useEffect(() => {
    const fetchDailyImportData = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:3001/api/restocks?month=${selectedMonth}&year=${selectedYear}`, {
            headers: {
                'Authorization': `Bearer ${token}` // Include the token in the headers
            }
        });
